...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  useEffect(() => {
    reportThrottleError(error, { boundary: "tanstack_root_error_component" });
  }, [error]);
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    ...
    links: [
      ...
      { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Outfit:wght@700;800&display=swap" },
...
function RootComponent() {
  ...
  const router = useRouter();
  useEffect(() => {
    // Lazy import to keep this browser-only.
